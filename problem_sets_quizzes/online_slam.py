...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    #motion_noise = 1
    #measurement_noise = 1
    num_dimensions = (num_landmarks + 1) * 2 #landmarks + 1 initial location

    main_omega_x = matrix()
    main_omega_x.zero(num_dimensions, num_dimensions)

    main_omega_x.value[0][0] = 1.
    main_omega_x.value[1][1] = 1.

    xi_x = matrix()

    xi_x.zero(num_dimensions, 1)
    
    xi_x.value[0][0] = world_size/2.
    xi_x.value[1][0] = world_size/2.

    for k, measurement in enumerate(data):
        lm_measurement = measurement[0]
        move_x = measurement[1][0]
        move_y = measurement[1][1]
        print 'dx: %.2f dy: %.2f' %(move_x, move_y)
        
        fill = [i for i in range(4,num_dimensions+2)]
        
        expand_omega = main_omega_x.expand(num_dimensions+2, num_dimensions+2, [0,1] + fill, [0,1] + fill)
        expand_xi = xi_x.expand(num_dimensions+2, 1, [0,1] + fill, [0])
        ...
        #movement
        #x-values
        expand_omega.value[0][0] += 1./motion_noise
        expand_omega.value[0][2] += -1./motion_noise
        expand_xi.value[0][0] += -move_x/motion_noise

        expand_omega.value[2][0] += -1./motion_noise
        expand_omega.value[2][2] += 1./motion_noise
        expand_xi.value[2][0] += move_x/motion_noise

        #y-values
        expand_omega.value[1][1] += 1./motion_noise
        expand_omega.value[1][3] += -1./motion_noise
        expand_xi.value[1][0] += -move_y/motion_noise

        expand_omega.value[3][1] += -1./motion_noise
        expand_omega.value[3][3] += 1./motion_noise
        expand_xi.value[3][0] += move_y/motion_noise

        if k == 0:
            expand_omega.show()
            expand_xi.show()

        for [lm_idx, lm_x, lm_y] in lm_measurement:
            print 'landmark idx: %s lm_x: %.2f lm_y: %.2f' %(lm_idx, lm_x, lm_y)

            lm_idx = 2 * lm_idx

            #x values
            expand_omega.value[0][0] += 1./measurement_noise
            expand_omega.value[0][4+lm_idx] += -1./measurement_noise
            expand_xi.value[0][0] += -lm_x/measurement_noise
            print 'adjusting 2, %s' %(4+lm_idx)

            expand_omega.value[4+lm_idx][0] += -1./measurement_noise
            expand_omega.value[4+lm_idx][4+lm_idx] += 1./measurement_noise
            expand_xi.value[4+lm_idx][0] += lm_x/measurement_noise
            
            #y values
            expand_omega.value[1][1] += 1./measurement_noise
            expand_omega.value[1][4+lm_idx+1] += -1./measurement_noise
            expand_xi.value[1][0] += -lm_y/measurement_noise
            print 'adjusting 3, %s' %(4+lm_idx+1)

            expand_omega.value[4+lm_idx+1][1] += -1./measurement_noise
            expand_omega.value[4+lm_idx+1][4+lm_idx+1] += 1./measurement_noise
            expand_xi.value[4+lm_idx+1][0] += lm_y/measurement_noise
        ...
        #collapsing back to original omega size
        A = expand_omega.take([0,1], [i for i in range(2, num_dimensions+2)])
        B = expand_omega.take([0,1], [0,1])
        C = expand_xi.take([0,1], [0])
        
        omega_prime_fill = [i for i in range(2, num_dimensions+2)]
        omega_prime = expand_omega.take(omega_prime_fill, omega_prime_fill) 

        xi_prime = expand_xi.take(omega_prime_fill, [0]) 

        main_omega_x = omega_prime - A.transpose() * B.inverse() * A
        xi_x = xi_prime - A.transpose() * B.inverse() * C
        

    Omega = main_omega_x
    mu = main_omega_x.inverse() * xi_x
