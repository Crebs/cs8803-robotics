...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    dimension = N + num_landmarks

    #omega_x = matrix()
    #omega_x.zero(dimension, dimension)

    omega = matrix()
    omega.zero(dimension, dimension)
    omega.value[0][0] =1.
    
    initial_data = [[0.] for j in range(dimension)]
    initial_data[0][0] = 50.

    initial_data2 = [[0.] for j in range(dimension)]
    initial_data2[0][0] = 50.

    xi_x = matrix(value=initial_data)
    xi_y = matrix(value=initial_data2)
    #intial_data_x[0][0] = data[0][
    #xi_x += matrix([

    for idx,step in enumerate(data):
        landmark_measurement = step[0]
        move_x = step[1][0]
        move_y = step[1][1]

        addition_omega = [[0. for x in range(dimension)] for y in range(dimension)]
        addition_omega[idx][idx] = 1./motion_noise
        addition_omega[idx][idx+1] = -1./motion_noise
        addition_omega[idx+1][idx] = -1./motion_noise
        addition_omega[idx+1][idx+1] = 1./motion_noise

        addition_omega = matrix(addition_omega)

        #import pdb
        #pdb.set_trace()

        omega += addition_omega

        addition_xi = [[0] for _ in range(dimension)]
        addition_xi[idx][0] = -1. * move_x / motion_noise
        addition_xi[idx+1][0] =  move_x / motion_noise
        #print 'move x: %s'%move_x
        xi_x += matrix(addition_xi)
        
        addition_xi = [[0] for _ in range(dimension)]
        addition_xi[idx][0] = -1 *move_y / motion_noise
        addition_xi[idx+1][0] =  move_y / motion_noise

        xi_y += matrix(addition_xi)

        for landmark in landmark_measurement:
            ...
            addition_omega = [[0. for x in range(dimension)] for y in range(dimension)]
            addition_omega[idx][idx] = 1/measurement_noise
            addition_omega[idx][N+landmark_idx] = -1/measurement_noise
            addition_omega[N+landmark_idx][idx] = -1/measurement_noise
            addition_omega[N+landmark_idx][N+landmark_idx] = 1/measurement_noise

            omega += matrix(value=addition_omega)


            addition_xi = [[0] for _ in range(dimension)]
            addition_xi[idx][0] = -1 * lm_x / measurement_noise
            addition_xi[N+landmark_idx][0] =  lm_x / measurement_noise

            xi_x += matrix(addition_xi)
            
            addition_xi = [[0] for _ in range(dimension)]
            addition_xi[idx][0] = -1 * lm_y / measurement_noise
            addition_xi[N+landmark_idx][0] = lm_y / measurement_noise

            xi_y += matrix(addition_xi)
    #print omega
    
    mu_x = omega.inverse() * xi_x
    mu_y = omega.inverse() * xi_y
    
    new_value = []
    for i in range(len(mu_x.value)):
        new_value.append(mu_x.value[i])
        new_value.append(mu_y.value[i])
