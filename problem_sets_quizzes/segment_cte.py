...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        [robot_x,robot_y, _] = estimate 
        [x,y] = spath[index]
        [next_x, next_y] = spath[index+1]
        u = (robot_x-x) * (next_x - x) + (robot_y-y) * (next_y - y)
        u /=  (next_x - x)**2 + (next_y - y)**2


        cte = (robot_y-y) * (next_x-x) - (robot_x-x) * (next_y-y)
        cte /=  (next_x - x)**2 + (next_y - y)**2   
        ...
        print "robot is at (%.2f,%.2f) with orientation %.2f moving along (%.2f,%.2f) and (%.2f, %.2f) with cte %.2f" %(estimate[0], estimate[1], estimate[2] * (180/pi), spath[index][0], spath[index][1], spath[index+1][0], spath[index+1][1], cte)
        ...
        print 'steering angle: %.2f' %(steer * 180./pi)
