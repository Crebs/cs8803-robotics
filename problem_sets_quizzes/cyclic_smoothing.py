...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    newpath = [element for element in path]
    error = 1
    while error > tolerance:
        for idx, i in enumerate(newpath):
            #if idx in [0, len(newpath)-1]:
            #    continue
            x,y = i
            new_y = weight_data * (path[idx][1] - y) + weight_smooth* (newpath[(idx+1)%len(newpath)][1] + newpath[idx-1][1] - 2*y)
            new_x = weight_data * (path[idx][0] - x) + weight_smooth* (newpath[(idx+1)%len(newpath)][0] + newpath[idx-1][0] - 2*x)
            newpath[idx] = [x+new_x, y+new_y]
            error = abs(new_y) + abs(new_x)
    # 
    # Enter code here
    #
